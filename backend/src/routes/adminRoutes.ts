...
export function adminRouter(deps?: { botRunner?: BotRunner }) {
  ...
  router.get('/plans', async (_req, res, next) => {
    try {
      ...
      return res.json(plans.map((p) => ({ id: String(p._id), name: p.name, durationMinutes: p.durationMinutes, amountKobo: p.amountKobo, currency: p.currency, videoUrl: p.videoUrl, description: p.description, isActive: p.isActive, createdAt: p.createdAt, updatedAt: p.updatedAt })));
    } catch (err) { return next(err); }
  });
  ...
  router.post('/plans', async (req, res, next) => {
    try {
      ...
      return res.status(201).json({ id: String(plan._id), name: plan.name, durationMinutes: plan.durationMinutes, amountKobo: plan.amountKobo, currency: plan.currency, videoUrl: plan.videoUrl, description: plan.description, isActive: plan.isActive });
    } catch (err) { return next(err); }
  });
  ...
  router.patch('/plans/:id', async (req, res, next) => {
    try {
      ...
      return res.json({ id: String(plan._id), name: plan.name, durationMinutes: plan.durationMinutes, amountKobo: plan.amountKobo, currency: plan.currency, videoUrl: plan.videoUrl, description: plan.description, isActive: plan.isActive, createdAt: plan.createdAt, updatedAt: plan.updatedAt });
    } catch (err) { return next(err); }
  });
